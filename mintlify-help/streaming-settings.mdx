...
Streaming transcription converts your speech to text in real time, word by word. It is separate from the mode-based recording system. You start it with its own keyboard shortcut, or with push-to-talk on macOS, Windows, and Linux. It uses its own provider and language settings.
...
On macOS, Windows, and Linux, enable push-to-talk in **Settings → Shortcuts**, choose the held key, and turn on **Use streaming for push-to-talk**. This setting is also available on the **Streaming** page. For example, choose **Right Option** on macOS or **Right Alt** on Windows/Linux.

Hold the key to stream with the provider selected in Streaming. Release it to finish and deliver the final text. Turn off **Double-press to lock** if you only want hold-to-talk. The dedicated streaming shortcut still toggles recording. If Streaming is off, push-to-talk uses regular recording. On Linux, click **Save settings** to apply your choice.

On Windows and Linux, held streaming displays text in the live preview and pastes it once you release the key. This avoids sending Ctrl+V while your push-to-talk modifier is still held.
...
| --- | --- |
| macOS | All nine |
| Windows | HyperWhisper Cloud, Deepgram, ElevenLabs, OpenAI, SpaceXAI, Soniox, Gemini 3.5 Transcribe |
| Linux | All nine |
...
Soniox streams directly with your Soniox API key and uses the `stt-rt-v5` model. Add the key in **Model Library → API Keys**, then select **Soniox** in the streaming provider picker.

You see provisional text while speaking. Confirmed utterances are inserted when Soniox detects an endpoint. Stopping streaming waits for the remaining confirmed text before closing the connection. Vocabulary hints work with an explicit language or **Automatic**.
