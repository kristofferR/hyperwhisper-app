...
Streaming transcription converts your speech to text in real time, word by word. It is separate from the mode-based recording system. You start it with its own keyboard shortcut, or with push-to-talk on macOS. It uses its own provider and language settings.
...
<Tabs>
  <Tab title="macOS">
    ...
    For hold-to-talk on macOS, enable **Push to Talk** in **Settings → Shortcuts**, select **Right Option** (or another trigger), and turn on **Use streaming for push-to-talk**. This toggle is also available in **Streaming**. Hold the selected key to stream with your streaming provider, then release it to finish. The dedicated streaming shortcut keeps its start/stop behavior. If you turn Streaming off, push-to-talk returns to regular recording.
